...
/// <summary>
/// 生成燃煤汽轮机第三层包装预制体、增量登记关键环节目录，并装配燃煤业务场景。
/// 包装接入统一动态播放协议：故障停止，其他状态默认播放；RanMeiManager 的组合特效总控
/// 和轴旋转控制器由燃煤专用适配器整体控制。
/// </summary>
public static class CoalSteamTurbineProcessDetailPrefabBuilder
{
    public const string SourcePrefabPath = "Assets/Prefabs/RanMeiManager.prefab";
    public const string OutputFolderPath = "Assets/ProcessDetails/CoalPower/SteamTurbine";
    public const string OutputPrefabPath = OutputFolderPath + "/CoalSteamTurbineProcessDetail.prefab";
    ...
    private const string ProcessDetailId = "process-detail.coal-power.steam-turbine";
    private const string ResourceId = "process-detail-resource.coal-power.steam-turbine";
    private const string CameraPoseId = "camera-pose.coal-power.steam-turbine";
    private const string StateNodeId = "node.coal-steam-turbine";
    private const string LegacyBoilerProcessDetailId = "process-detail.coal-power.boiler";

    // 第三层展示区与二层厂区保持显式空间隔离，满足协调器至少 1000 米的远端距离校验。
    private static readonly Vector3 RemoteDisplayPosition = new Vector3(10000f, 0f, 0f);

    [MenuItem("Tools/WebDLPro/关键环节/生成燃煤汽轮机第三层资源")]
    public static void CreateOrUpdate()
    {
        ...
        {
            throw new InvalidOperationException("缺少 RanMeiManager 源预制体或设备四态视觉配置。");
        }
        ...
        Debug.Log($"[ProcessDetailBuilder] 已生成燃煤汽轮机第三层资源：{OutputPrefabPath}");
    }
    ...
    private static GameObject CreateWrapperPrefab(
        ...
        PowerPlantVisualStateConfig visualConfig)
    {
        GameObject host = new GameObject("CoalSteamTurbineProcessDetail");
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("无法创建 RanMeiManager 源预制体的嵌套实例。");
            }
            ...
            CoalPowerSteamEffectsController[] effectControllers =
                modelInstance.GetComponentsInChildren<CoalPowerSteamEffectsController>(true);
            CoalPowerShaftRotationController[] shaftRotationControllers =
                modelInstance.GetComponentsInChildren<CoalPowerShaftRotationController>(true);
            if (effectControllers.Length == 0 || shaftRotationControllers.Length == 0)
            {
                throw new InvalidOperationException(
                    "RanMeiManager 源预制体缺少蒸汽特效总控或轴旋转控制器。");
            }

            CoalSteamTurbineProcessDetailDynamicAdapter dynamicAdapter =
                host.AddComponent<CoalSteamTurbineProcessDetailDynamicAdapter>();
            dynamicAdapter.ConfigureForEditor(effectControllers, shaftRotationControllers);
            ...
            {
                throw new InvalidOperationException("燃煤汽轮机包装未收集到可用于四态视觉的设备渲染器。");
            }
            ...
            if (prefab == null)
            {
                throw new InvalidOperationException("燃煤汽轮机第三层包装预制体保存失败。");
            }
            return prefab;
        }
        finally
    ...
    /// <summary>
    /// 编辑器只扫描一次源实例并固化渲染器数组。粒子渲染器与运行时生成的能量、电子流覆盖层
    /// 不属于 MeshRenderer 扫描结果；同时跳过不支持基础色属性的材质，避免状态投影破坏特效材质。
    /// </summary>
    ...
    private static ProcessDetailCatalog CreateOrUpdateCatalog(GameObject wrapperPrefab)
    {
        ...
        for (int index = 0; index < catalog.Entries.Count; index++)
        {
            ...
            if (string.Equals(existing?.ProcessDetailId, ProcessDetailId, StringComparison.Ordinal) ||
                string.Equals(existing?.ProcessDetailId, LegacyBoilerProcessDetailId, StringComparison.Ordinal))
            {
                entries.Add(entry);
                replaced = true;
            }
