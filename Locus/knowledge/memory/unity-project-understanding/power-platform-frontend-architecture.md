...
- 当前桥接的 `ack` 和 `commandResult` 已通过 `payload.requestId` 关联原始命令；今后须分离命令/事件能力，且所有异步消息按唯一 `instanceId` 过滤。三维容器右上角提供独立 `resetCamera` 命令按钮：平滑恢复当前场景初始镜头，同时撤销流程、命名镜头和交互产生的临时视觉并恢复总览显隐；设备当前四态及故障停流效果保留，成功后复用 `selectionCleared` 通知前端清空拓扑选择。该命令不复用会改写流程字段的 `resetScene`。
