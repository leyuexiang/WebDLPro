...
7. Unity 源码已经实现 `switchScene`、`sceneLoadProgress`（场景加载进度）、`sceneChanged`（场景切换完成）、失败自动恢复和物理场景激活标识；但当前可用正式网页图形基线仍是旧协议构建，必须重新构建后才能用于当前三层联调。
...
| `setNodeVisualState` | `sceneNodeId`、四态状态、`statusUpdatedAt`、`hasSourceRevision`、`sourceRevision` | 增量同步设备状态；时间与来源修订组成因果水位，同时间只允许更高来源修订覆盖 |
...
当前版本化协议元数据采用结构版本 2，并与同一 Unity 发布标识绑定。它至少声明：内层信封协议版本 1、`sceneChanged` 结构版本 2、场景失败恢复结构版本 1、`setNodeVisualState` 结构版本 2，以及下列必填字段集合：
...
- `setNodeVisualState`：`sceneNodeId`、`visualState`、`statusUpdatedAt`、`hasSourceRevision`、`sourceRevision`。

元数据只能在 Unity 播放器成功构建后写入，并随同一暂存目录原子固化。前端发布门禁只允许精确版本和必填字段均匹配的 Unity 基线继续打包。当前源码与自动测试已经实现该契约，但新版正式网页图形产物尚未构建，不能把源码能力声明当作真实发布兼容性证据。
...
| 三维状态因果水位 | 按当前活动控制器和三维节点有限保存；更早时间、同时间低修订及重复重试不得覆盖较新状态，场景控制器变化和整体释放时清空 |
