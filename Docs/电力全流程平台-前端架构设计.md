...
7. 部分正式拓扑节点可在当前拓扑面板内打开全尺寸说明覆盖层，展示不属于总拓扑事实的内部关联；该说明层不进入正式拓扑切换、Unity 或外层通信链路。
...
| 总览重点区域 | `focusRegions` 配置与 `CanvasTopologyAdapter.drawFocusRegions` | 只在总览按显式三维锚点及子节点绘制稳定配色虚线区域框；过滤拓扑清空，不改变节点命中、路由或联动 |
| 下钻说明层 | 计划新增 `TopologyDrilldownOverlay.vue` 与只读内容注册表 | 挂载在拓扑面板内部，使用可缩放矢量图形与文档对象模型，保持单画布和正式联动状态隔离 |
...
2. 三维容器和拓扑容器各占可用高度的一半；两者之间仅保留 1 像素分隔。三维实际视口的高度等于“父容器可用宽度按 16:9 换算的高度”与“三维半区高度”中的较小值；它不是 `1280×720` 等固定像素尺寸，也不设置固定像素上限。
3. 拓扑外层固定占据下半区；其内层宽度必须等于三维实际视口宽度，标题、图例、画布和全屏入口与 Unity 左右边界对齐。超宽屏两侧未占用区域统一使用三维区纯黑基底，不渲染拓扑容器的青色背景。
4. 常规态不显示节点、连线和设备状态统计摘要。拓扑画布直接使用标题、图例和内边距之外的全部下半区高度；空拓扑才显示明确的配置缺失说明。外层低于部署阈值时显示结构化“容器尺寸不足”。
5. 两个常规态容器使用等分网格行，不能用内容高度持续撑开页面。
6. 推荐父页面提供不小于 `1280 × 720` 的可用区域；高度不足时压缩到各自最小高度，低于最低可用高度时显示结构化“容器尺寸不足”。
7. 三维和拓扑全屏必须使用同一套浏览器原生全屏接口：分别把三维视口或拓扑面板登记为当前全屏元素，从平台容器直接进入浏览器顶层全屏。父平台及中间每层内嵌框架必须声明 `allow="fullscreen"` 或等效权限；缺少权限时保持常规态并报告接入配置问题，禁止降级为只能覆盖平台容器的固定定位遮罩。拓扑全屏时标题、图例和内边距占用自身高度，唯一画布填充其余全部高度；节点图元、节点标题、层级标题和连线文字均为常规态的 2 倍，且同步扩大避让与命中几何，不改写用户缩放和平移。通过浏览器原生 `Esc` 或关闭按钮退出后恢复常规展示倍率。
8. 场景或拓扑切换期间使用统一遮罩，禁止用户在“旧场景 + 新拓扑”或“新场景 + 旧拓扑”组合上继续操作。

所有后续拓扑必须采用[拓扑容器与多图切换规范](前端原子任务/拓扑图/拓扑容器与多图切换规范.md)中的已确认制作基线：分层、连线证据、协议标签、节点图元、状态、文字避让和画布性能均由明确配置与受控运行时状态驱动，不复制燃气专有业务数据。节点视觉尺寸由画布宽高、最密层节点数和相邻层净距共同计算，常规图标约28—40像素，紧凑及极低画布允许继续缩小；节点直接使用正式图元，未选中时不叠加常驻节点外框，选中时只叠加独立选中反馈，路由安全边界和点击边界才合并图元与下方文字。点击区域独立保持至少40×40像素，重叠时取距离节点中心最近者。
...
| 节点视觉 | 拟物化 Cisco 风图元直接作为节点主体；文字位于图元下方；未选中节点不绘制常驻外框，选中时叠加独立青白色选中反馈和轻微光晕；不绘制右上角状态圆点；常规图标约28—40像素，命中区至少40×40像素 |
| 重点区域 | 总览显式登记三组区域；区域框使用成员视觉边界、轻透明填充、虚线和稳定伪随机颜色；关键环节不显示，区域框不参与命中或路由 |
| 连线路由 | 任意角度直线优先；仅在穿过第三方完整节点边界、共线重叠或非端点交叉时按最少拐点、最短长度绕行 |
| 二维到三维 | 节点单击先提交二维选择，再对显式 `sceneNodeId` 发送 `focusNode`；三维失败不回滚二维选择；是否自动聚焦由 Unity 的 `_focusOnSelection` 统一开关控制 |
| 空白清除 | 拓扑空白先清除二维节点和关联边，再发送 `clearSelection`；Unity 空白/地面/未映射对象则上行 `selectionCleared` 清除二维选择；两者都不改变流程、显隐、告警或已到达镜头 |
...
interface ActionDefinition {
  ...
  unityAction:
    ...
    | { type: 'focusNode'; sceneNodeId: string; selectionId: string; isolate: boolean }
    | { type: 'resetScene' }
}
...
| `focusNode` | `sceneNodeId`、`selectionId`、`isolate` | 更新三维交互描边；由 `_focusOnSelection` 统一开关决定是否平滑聚焦 |
...
| `objectSelected` | 且仅允许 `sceneId`、`sceneNodeId`、`sceneActivationId` | Unity 鼠标命中已登记三维节点；去重使用信封 `messageId`，禁止携带对象名称、二维节点或任意扩展字段 |
| `selectionCleared` | 且仅允许 `sceneId`、`sceneActivationId` | Unity 鼠标点击空白、地面或未映射对象时清除交互选择；前端清空二维节点和关联路径，不回发 `clearSelection` |
...
| --- | --- | --- | --- |
| 单击节点 | 立即选中、显示独立选中反馈并高亮关联路径 | 有 `sceneNodeId` 时发送 `focusNode` | 默认不上报 |
| 双击节点 | 保持单击选中结果 | 不重复发送相同聚焦命令 | 发送 `topology.node.dblclick`，`nodeId` 必填 |
| 单击画布空白 | 清除二维节点及关联路径选择 | 发送 `clearSelection`，只清除三维交互描边 | 默认不上报 |
| Unity 鼠标命中已登记节点 | 前端按 `sceneNodeId → nodeId` 同步二维节点及关联路径 | 更新三维交互描边；按 `_focusOnSelection` 决定是否平滑聚焦 | 不回发 `focusNode` |
| Unity 鼠标点击空白、地面或未映射对象 | 当前场景实例有效时清除二维节点及关联路径 | 清除交互描边并发送 `selectionCleared` | 不回发 `clearSelection` |
...
- 总览重点区域使用独立 `focusRegions[]` 声明：`regionId` 唯一，`anchorNodeId` 必须有 `sceneNodeId`，`nodeIds` 必须全部存在且包含锚点；过滤视图不得复制或继承区域声明。
- 二维选中反馈使用独立叠加层：选中时显示高对比外圈或轻微光晕，未选中时不显示常驻装饰；反馈不改写四态图元、不改变重点区域边界、不扩大命中或路由边界。

### 9.4 说明型下钻覆盖层

说明型下钻与正式多拓扑运行时并列、隔离：正式入口节点只保存可选内容引用，说明节点保存在独立版本化资源中。打开下钻不创建 `transitionId`，不改变当前稳定 `sceneId`、`topologyId`、`actionId` 和选择状态，也不调用 Unity 或外层桥。

覆盖层与原拓扑内容作为拓扑面板内部同一相对定位工作区的兄弟节点。覆盖层四边贴合面板并使用不透明背景；原内容保留唯一画布但进入不可交互状态。少量说明节点和连线使用可缩放矢量图形与文档对象模型绘制，不创建第二个画布。燃煤、燃气下钻的来源节点、直接子节点和模型节点均使用正式图元登记表中的受控 `iconKey`，标题位于图标下方；禁止纯文字节点、额外节点卡片和动态图片路径。静态说明图统一使用正常图标，不接入告警、故障或离线状态。窗口缩放、嵌入区域变化和拓扑原生全屏都由容器尺寸自动驱动，不保存页面级屏幕坐标。

下钻入口必须与单击、双击分离。单击继续负责正式二维选择和可选三维聚焦，双击继续上报正式 `nodeId`；下钻按钮只更新覆盖层局部状态。说明节点不拥有正式 `nodeId`、`sceneNodeId`、四态状态或平台绑定，关闭后恢复原拓扑的缩放、平移、选择、路径与焦点。完整实施和验收边界见[拓扑下钻关联覆盖层实施计划](前端原子任务/拓扑图/拓扑下钻关联覆盖层实施计划.md)。
...
| 组件测试 | 两容器布局、多拓扑切换、单击、双击、全屏、下钻覆盖层尺寸与交互隔离、错误态 |
| Unity 编辑器测试 | 九场景登记、加载、卸载、流程动作、聚焦、复位、释放、强制同场景重载、失败恢复激活标识、对象选择最小负载和迟到状态阻断 |
| 三层联调测试 | 外部宿主页 → 当前子应用 → Unity 模拟页 |
| 端到端测试 | 九场景初始进入、同场景子项、跨场景切换、节点双击上报、下钻打开关闭期间消息与Unity命令零新增 |
| 性能测试 | 首屏、场景切换、拓扑切换、下钻重复打开关闭与尺寸变化、长时间运行、内存和显存 |
...
15. 下钻覆盖层完全遮挡拓扑面板并自动跟随常规态、紧凑态和原生全屏尺寸；打开期间保持一个拓扑画布和一个Unity实例。
16. 下钻说明节点不进入正式拓扑、状态、三维映射、平台绑定和外层消息；关闭后恢复原拓扑视图与焦点，重复打开关闭不累积监听器或缓存。
