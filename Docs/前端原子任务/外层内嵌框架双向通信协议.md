...
6. `system.ready` 只返回清单版本、场景目录和能力摘要；完整结构清单继续从同源 `/scene-topology-manifest.json` 读取，不通过跨窗口消息复制。
...
`system.ready` 的载荷是结构清单摘要，不包含完整结构清单。该事件提前发送不会解除启动遮罩，也不表示 Unity 已可执行命令；平台应使用其中的 `manifestVersion` 和能力声明构造唯一 `system.init`。
