...
5. 子应用必须在页面加载后 15 秒内完成结构清单校验、唯一拓扑运行时和 Unity 运行时准备，并发送 `system.ready`；否则进入明确启动错误态，向已校验父窗口发送一次无 `replyTo` 的脱敏 `system.error` 后释放当前桥接，迟到回调不得补发 `system.ready`。
6. 发出 `system.ready` 后可以继续等待一次合法 `system.init`；父页面初始化等待不与前述 15 秒就绪期限混为同一超时。
7. `system.init` 失败时，子应用必须同时发送关联的 `system.ack success:false` 和 `system.error`，两者使用相同 `replyTo`。
...
对 `device.states.update`，`completed` 的业务边界固定为“载荷校验通过，完整快照已经替换并完成二维投影”。只有协议校验失败或二维提交失败才返回 `failed`。三维状态同步可以在外层成功回执后异步完成，其失败不得改变、补发或撤销该回执。
...
| `runtime.startup.timeout` | 页面未在 15 秒启动期限内完成清单、拓扑与三维运行时准备 |
...
9. `contextRevision` 每次成功提交场景、拓扑或动作上下文后递增。
...
- [x] 子应用在加载后 15 秒内完成结构清单、拓扑和三维运行时准备并发送 `system.ready`；超时发送一次脱敏错误且禁止迟到就绪；初始化失败同时发送失败确认和结构化错误。
...
- [ ] 未识别设备被忽略，二维快照成功提交后外层仍只返回一次成功结果。
