...
describe('TopologySelectionFocusCoordinator', () => {
  it('只将拓扑来源且显式映射的节点聚焦一次', async () => {
    ...
    const request = { source: 'topology' as const, correlationId: 'topology-select-01', sceneNodeId: toSceneNodeId('scene-node.gas-turbine') }
    ...
    expect(port.focusNode).toHaveBeenCalledWith(toSceneNodeId('scene-node.gas-turbine'))
  })
  ...
  it('Unity 来源、缺少映射或未协商能力均不向三维回写', async () => {
    ...
    await expect(coordinator.requestFocus({ source: 'unity', correlationId: 'unity-select-01', sceneNodeId: toSceneNodeId('scene-node.gas-turbine') })).resolves.toBe(false)
    await expect(coordinator.requestFocus({ source: 'topology', correlationId: 'topology-select-no-node' })).resolves.toBe(false)
    await expect(coordinator.requestFocus({ source: 'topology', correlationId: 'topology-select-no-capability', sceneNodeId: toSceneNodeId('scene-node.gas-turbine') })).resolves.toBe(false)
  ...
  it('三维聚焦失败不撤销已处理关联，重放同一关联不会重复发送命令', async () => {
    ...
    const coordinator = new TopologySelectionFocusCoordinator(port)
    const request = { source: 'topology' as const, correlationId: 'topology-select-failed', sceneNodeId: toSceneNodeId('scene-node.gas-turbine') }
