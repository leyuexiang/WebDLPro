...
import { getSwitchingStationTopologyResourceManifest } from './switching-station-topology-manifest'
import {
  applySwitchingStationTopologySelectionPolicy,
  removeSwitchingStationFullTopologyRootCombine,
} from './switching-station-topology-selection'
import {
  SWITCHING_STATION_TOPOLOGY_VARIANT_BY_ID,
  type SwitchingStationTopologyVariantId,
} from './switching-station-topology-variant-manifest'

/** 未交给二维组态引擎的安全源副本按版本缓存；再次切回不重复请求或解析。 */
const sourceDataCache = new Map<SwitchingStationTopologyVariantId, Meta2dData>()

/** 生成开关站版本数据地址；图片始终通过公共资源工具生成，不访问源包站点路径。 */
export function getSwitchingStationTopologyPublicAssetUrl(
  relativePath: string,
  ...
  const assetRelativePath = `topology/switching-station-json-preview/${relativePath}`
 ...
 */
function localizePenImage(pen: Pen, variantId: SwitchingStationTopologyVariantId): void {
  ...
  if (!staticPath) throw new Error(`开关站拓扑图片图元未登记：${variantId}/${pen.id}`)
  pen.image = getTopologySharedPublicAssetUrl(staticPath)
}

/** 图元数、编号和有限几何字段在进入画布前逐项校验，防止异常边界污染适应视图。 */
function validateSwitchingStationTopologyPens(
  pens: readonly Pen[],
  variantId: SwitchingStationTopologyVariantId,
  expectedPenCount: number,
): void {
  if (pens.length !== expectedPenCount) {
    throw new Error(`开关站拓扑 ${variantId} 图元数量异常：应为 ${expectedPenCount}，实际为 ${pens.length}。`)
  }
  const ids = new Set<string>()
  for (const pen of pens) {
    if (!pen.id) throw new Error(`开关站拓扑 ${variantId} 格式无效：存在无编号图元。`)
    if (ids.has(pen.id)) throw new Error(`开关站拓扑 ${variantId} 格式无效：图元编号重复 ${pen.id}。`)
    ids.add(pen.id)
    if (![pen.x, pen.y, pen.width, pen.height].every((value) => typeof value === 'number' && Number.isFinite(value))) {
      throw new Error(`开关站拓扑 ${variantId} 格式无效：图元 ${pen.id} 的坐标或尺寸不是有限数字。`)
    }
  }
}
 ...
 */
export async function loadSwitchingStationTopologyPreviewData(
  variantId: SwitchingStationTopologyVariantId = 'network-business-key-process',
  signal?: AbortSignal,
): Promise<Meta2dData> {
  const variant = SWITCHING_STATION_TOPOLOGY_VARIANT_BY_ID.get(variantId)
  if (!variant) throw new Error(`开关站拓扑版本未登记：${variantId}`)

  let sourceData = sourceDataCache.get(variantId)
  if (!sourceData) {
    const response = await fetch(getSwitchingStationTopologyPublicAssetUrl(variant.topologyPath), {
      signal,
      cache: import.meta.env.DEV ? 'no-store' : 'force-cache',
    })
    if (!response.ok) throw new Error(`开关站拓扑 ${variantId} 加载失败：${response.status}`)
    const fetchedData = await response.json() as Partial<Meta2dData>
    if (!Array.isArray(fetchedData.pens)) throw new Error(`开关站拓扑 ${variantId} 格式无效：缺少图元数组。`)
    validateSwitchingStationTopologyPens(fetchedData.pens, variantId, variant.expectedPenCount)
  ...
  removeSwitchingStationFullTopologyRootCombine(data.pens, variantId)
  applySwitchingStationTopologySelectionPolicy(data.pens, variantId)
...
export function clearSwitchingStationTopologyPreviewDataCacheForTests(): void {
