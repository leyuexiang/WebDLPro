...
describe('开关站拓扑版本清单', () => {
  it('八个组合各自指向散列和图元数匹配的独立文件', () => {
    expect(SWITCHING_STATION_TOPOLOGY_VARIANTS).toHaveLength(8)
  ...
  it('架构层独立且只有三层完整输入是默认文件', () => {
    const architecture = SWITCHING_STATION_TOPOLOGY_VARIANTS.find((variant) => variant.id === 'architecture')
    expect(architecture?.layerIds).toEqual(['architecture'])
    expect(SWITCHING_STATION_TOPOLOGY_VARIANTS
      .filter((variant) => variant.id !== 'architecture')
      .every((variant) => !(variant.layerIds as readonly string[]).includes('architecture'))).toBe(true)
    expect(SWITCHING_STATION_TOPOLOGY_VARIANTS.filter((variant) => 'isDefault' in variant))
