...
import { getWindTopologyTooltipContent, type WindTopologyTooltipContent } from './wind-topology-tooltip'
import WindTopologyLayerFilter from './WindTopologyLayerFilter.vue'
...
import type { WindTopologyVariantId, WindTopologyVariantManifestEntry } from './wind-topology-variant-manifest'
import {
...
const props = defineProps<{ fullscreenTarget?: HTMLElement | null; suspended?: boolean }>()
...
const activeTooltip = ref<WindTopologyTooltipContent | null>(null)
const tooltipX = ref(16)
const tooltipY = ref(68)
const selectedFilterIds = ref<ReadonlySet<WindTopologyFilterId>>(createDefaultWindTopologyFilterSelection())
const currentVariantId = ref<WindTopologyVariantId>('network-business-key-process')
...
const statusText = computed(() => loadingState.value === 'loading'
  ...
  : loadingState.value === 'error'
    ? errorMessage.value
    : `正在展示：${formatWindTopologyFilterSelection(selectedFilterIds.value)}`)
...
/** 窄接口仅暴露就绪状态与重置，不把引擎或尚未接入的设备状态能力暴露给父层。 */
defineExpose({
...
function handleTooltipPenEnter(pen?: Pen): void {
  activeTooltip.value = pen ? getWindTopologyTooltipContent(pen) ?? null : null
}
...
function commitTopologyVariant(
  data: Meta2dData,
  variant: WindTopologyVariantManifestEntry,
  revision: number,
): void {
...
async function switchTopologyVariant(variant: WindTopologyVariantManifestEntry): Promise<void> {
  ...
  if (hasCurrentCanvas) layerNotice.value = `正在切换到${formatWindTopologyFilterSelection(selectedFilterIds.value)}…`
  else loadingState.value = 'loading'
  try {
    const data = await loadWindTopologyPreviewData(variant.id, controller.signal)
    if (disposed || revision !== loadRevision || controller.signal.aborted) return
    commitTopologyVariant(data, variant, revision)
  } catch (error) {
    if (disposed || revision !== loadRevision || controller.signal.aborted) return
    const message = error instanceof Error ? error.message : '风电拓扑加载失败。'
...
function handleLayerFilterChange(filterId: WindTopologyFilterId, checked: boolean): void {
  const nextSelection = toggleWindTopologyFilter(selectedFilterIds.value, filterId, checked)
  selectedFilterIds.value = nextSelection
  const variant = resolveWindTopologyVariant(nextSelection)
  if (!variant) {
    ++loadRevision
    topologyRequestController?.abort()
    const missingMessage = nextSelection.size === 0
      ? '请至少选择一个有输入文件的层级。当前拓扑保持不变。'
      : `${formatWindTopologyFilterSelection(nextSelection)}暂无独立输入文件，当前拓扑保持不变。`
...
onMounted(async () => {
  ...
  const initialVariant = resolveWindTopologyVariant(selectedFilterIds.value)
...
<template>
  <main class="wind-topology-preview" :class="{ 'wind-topology-preview--embedded': props.fullscreenTarget }" :inert="props.suspended">
    <header class="wind-topology-preview__toolbar">
      <div class="wind-topology-preview__heading">
        <h1>风电拓扑图预览</h1>
        <p>{{ statusText }}</p>
      </div>
      <div class="wind-topology-preview__actions" aria-label="拓扑图视图控制">
    ...
    <section ref="canvasStage" class="wind-topology-preview__stage" aria-label="风电拓扑图画布">
      <WindTopologyLayerFilter :selected-filter-ids="selectedFilterIds" @change="handleLayerFilterChange" />
      <div
        ref="canvasHost"
        class="wind-topology-preview__canvas"
      ...
      <div v-if="activeTooltip" class="wind-topology-preview__tooltip" :style="tooltipStyle" role="tooltip">
        <strong>{{ activeTooltip.title }}</strong>
        <span>状态：{{ activeTooltip.status }}</span>
      </div>
      <p v-if="layerNotice && loadingState === 'ready'" class="wind-topology-preview__notice" role="status">
        {{ layerNotice }}
      </p>
      <div v-if="loadingState !== 'ready'" class="wind-topology-preview__state" role="status">{{ statusText }}</div>
    </section>
  </main>
</template>

<style scoped>
.wind-topology-preview {
  display: grid;
...
.wind-topology-preview--embedded { block-size: 100%; min-block-size: 0; }

.wind-topology-preview__toolbar {
...
.wind-topology-preview__heading h1 { margin: 0; font-size: 20px; line-height: 1.4; }
.wind-topology-preview__heading p { margin: 2px 0 0; color: #667085; font-size: 12px; }
.wind-topology-preview__actions { display: flex; align-items: center; gap: 8px; white-space: nowrap; }

.wind-topology-preview__actions button {
  min-block-size: 36px;
  padding: 7px 13px;
...
.wind-topology-preview__actions button:hover:not(:disabled) { border-color: #2563eb; color: #1d4ed8; }
.wind-topology-preview__actions button:focus-visible { outline: 3px solid rgba(37, 99, 235, 0.25); outline-offset: 2px; }
.wind-topology-preview__actions button:disabled { cursor: not-allowed; opacity: 0.45; }
.wind-topology-preview__actions output { inline-size: 58px; color: #475467; text-align: center; font-variant-numeric: tabular-nums; }

.wind-topology-preview__stage {
  position: relative;
  display: grid;
  grid-template-rows: minmax(0, 1fr);
  min-block-size: 0;
...
.wind-topology-preview__canvas { inline-size: 100%; block-size: 100%; overflow: hidden; background: #1e2430; }
.wind-topology-preview__stage:fullscreen { margin: 0; border: 0; border-radius: 0; }

.wind-topology-preview__tooltip {
  position: absolute;
  z-index: 25;
  display: grid;
...
.wind-topology-preview__tooltip strong { font-size: 12px; }
.wind-topology-preview__tooltip span { color: #9dd8e5; }

.wind-topology-preview__notice {
  position: absolute;
...
@media (max-width: 760px) {
  .wind-topology-preview__toolbar { align-items: flex-start; flex-direction: column; gap: 10px; padding: 10px 12px; }
  .wind-topology-preview__actions { inline-size: 100%; overflow-x: auto; padding-block-end: 2px; }
  .wind-topology-preview__stage { margin: 0; }
}
</style>
