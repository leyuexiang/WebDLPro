...
import { getGasV3TopologyStatusIconUrl, loadGasV3TopologyPreviewData } from './gas-v3-topology-preview-data'
...
import {
  ...
  type GasV3TopologyRuntimeBinding,
...
const currentBindings = shallowRef<readonly GasV3TopologyRuntimeBinding[]>(getGasV3TopologyRuntimeBindings(currentVariantId.value))
const bindingIndex = shallowRef(createGasV3TopologyRuntimeBindingIndex(currentVariantId.value))
...
const activeTooltip = computed<GasV3TopologyTooltipContent | null>(() => {
  ...
  return getGasV3TopologyTooltipContent(pen, getEffectiveNodeStatus(nodeId)) ?? null
})
...
let disposed = false
/** 返回第二层时恢复进入第三层前的筛选文件，而不是强制改回默认文件。 */
let lastBusinessVariantId: GasV3TopologyVariantId = 'network-business-key-process'
...
function applyRuntimeStatuses(render = true): void {
  ...
  for (const binding of currentBindings.value) {
    ...
    const image = getGasV3TopologyStatusIconUrl(currentVariantId.value, binding.penId, status)
    if (!image) continue
...
function commitTopologyVariant(
  data: Meta2dData,
  variant: GasV3TopologyVariantManifestEntry | GasV3ProcessDetailVariantManifestEntry,
  ...
  bindingIndex.value = createGasV3TopologyRuntimeBindingIndex(variant.id)
...
async function switchTopologyVariant(
  ...
  if (variant.id !== 'process-detail-gas-turbine') lastBusinessVariantId = variant.id
  const hasCurrentCanvas = loadingState.value === 'ready'
  if (hasCurrentCanvas) {
    layerNotice.value = 'isProcessDetail' in variant
      ? '正在加载当前关键环节拓扑…'
      : `正在切换到${formatGasV3TopologyFilterSelection(selectedFilterIds.value)}…`
  }
  ...
  try {
    ...
    const projectedStatuses = projectGasV3TopologyStatusesBeforeOpen(
      data,
      variant.id,
      bindings,
...
/**
 * 第三层上下文只选择已登记的关键环节文件；清除上下文时恢复进入前的业务层文件。
 * 该方法不创建新组件、不复制 Meta2D 实例，只替换同一画布的数据源。
 */
function setTopologyDataContext(context: TopologyDataContext | undefined): void {
  ...
  if (!context) {
    ...
    if (currentVariantId.value === 'process-detail-gas-turbine') {
      const variant = GAS_V3_TOPOLOGY_VARIANT_BY_ID.get(lastBusinessVariantId)
      if (variant && !('isProcessDetail' in variant)) void switchTopologyVariant(variant)
    }
    return
  }
  if (context.renderer !== 'gas-v3' || context.contextId !== 'process-detail.gas-power.gas-turbine') return
  // 第三层入口一旦确认有效便立即关闭筛选；拓扑内容只由该关键环节的独立文件决定。
  processDetailContextActive.value = true
  void switchTopologyVariant(GAS_V3_PROCESS_DETAIL_VARIANTS[0])
}
...
function handleLayerFilterChange(filterId: GasV3TopologyFilterId, checked: boolean): void {
  ...
  if (!variant) {
    ++loadRevision
    topologyRequestController?.abort()
    const selectionLabel = formatGasV3TopologyFilterSelection(nextSelection)
...
onMounted(async () => {
  ...
  if (!readUsableTopologyViewportSize(host)) {
    loadingState.value = 'error'
    errorMessage.value = '燃气拓扑画布可用尺寸为零。'
    return
  }
...
<template>
  ...
  >
    ...
    <p v-if="layerNotice && loadingState === 'ready'" class="gas-topology-runtime-canvas__notice" role="status">
      {{ layerNotice }}
    </p>
    <p v-if="loadingState !== 'ready'" class="gas-topology-runtime-canvas__state" role="status">
      {{ loadingState === 'error' ? errorMessage : '正在加载燃气拓扑…' }}
    </p>
  </div>
</template>
...
.gas-topology-runtime-canvas:focus-visible { box-shadow: inset 0 0 0 2px rgba(103, 232, 249, 0.72); }
.gas-topology-runtime-canvas:fullscreen { border: 0; border-radius: 0; }
.gas-topology-runtime-canvas__host { inline-size: 100%; block-size: 100%; overflow: hidden; background: #1e2430; }
...
.gas-topology-runtime-canvas__tooltip strong { font-size: 12px; }
.gas-topology-runtime-canvas__tooltip span { color: #9dd8e5; }

.gas-topology-runtime-canvas__notice {
  position: absolute;
