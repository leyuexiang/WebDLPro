...
export const GAS_TOPOLOGY_SELECTABLE_LOCK = 1 as LockState

/** 对应组态引擎的“完全禁用”：图元直接跳过鼠标命中，因此背景不会出现悬停或选中框。 */
export const GAS_TOPOLOGY_BACKGROUND_LOCK = 10 as LockState
...
/**
 * 应用燃气拓扑的命中规则，并返回允许选中的图元编号集合，便于测试和后续交互扩展。
 * 可选对象仅包括带图片的设备图元，以及图三所示的底部工艺流程小矩形；连线、分区标题和分组背景全部禁用命中。
 */
export function applyGasTopologySelectionPolicy(pens: Pen[]): ReadonlySet<string> {
  ...
  for (const pen of pens) {
    ...
    pen.locked = selectable ? GAS_TOPOLOGY_SELECTABLE_LOCK : GAS_TOPOLOGY_BACKGROUND_LOCK
