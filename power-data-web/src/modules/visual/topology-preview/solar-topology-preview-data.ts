...
import { applySolarTopologySelectionPolicy } from './solar-topology-selection'
...
export {
  SOLAR_TOPOLOGY_BACKGROUND_PEN_IDS,
  SOLAR_TOPOLOGY_DEVICE_PEN_IDS,
} from './solar-topology-manifest'
 ...
 */
const sourceDataCache = new Map<SolarTopologyVariantId, Meta2dData>()

/** 按版本和图元编号读取公共四态图标，不解析标题、坐标或输入文件的原图片地址。 */
export function getSolarTopologyPreviewIconPath(
  variantId: SolarTopologyVariantId,
  penId: string,
  status: TopologyDeviceStatus = 'normal',
): string | undefined {
  const normalPath = getSolarTopologyResourceManifest(variantId).deviceIconPathByPenId.get(penId)
  return normalPath?.replace('/normal/', `/${ICON_DIRECTORY_BY_STATUS[status]}/`)
}

/** 把正式设备状态转换为受控公共资源地址。 */
export function getSolarTopologyStatusIconUrl(
  variantId: SolarTopologyVariantId,
  penId: string,
  status: TopologyDeviceStatus,
): string | undefined {
  const relativePath = getSolarTopologyPreviewIconPath(variantId, penId, status)
  return relativePath ? getTopologySharedPublicAssetUrl(relativePath) : undefined
}

/** 生成光伏版本数据地址；图片资源始终由公共资源地址工具单独生成。 */
export function getSolarTopologyPreviewPublicAssetUrl(
  relativePath: string,
  ...
  const assetRelativePath = `topology/solar-json-preview/${relativePath}`
 ...
 */
function localizePenImage(pen: Pen, variantId: SolarTopologyVariantId): void {
  if (!pen.image?.trim() || !pen.id) return
  // 源数据登记的是公共资源键，不是部署地址。必须与燃气、燃煤、风电共用地址工具，
  // 让相对发布定位到 shell/topology/shared，子目录部署也不会错误请求站点根路径。
  if (pen.image.startsWith('/topology/shared/assets/')) {
    pen.image = getTopologySharedPublicAssetUrl(pen.image.slice('/topology/shared/'.length))
    return
  }
  const manifest = getSolarTopologyResourceManifest(variantId)
  if (manifest.titleBackgroundPenIds.has(pen.id)) {
    pen.image = getTopologySharedPublicAssetUrl('background/flow-light-3.png')
    return
  }
  const iconPath = getSolarTopologyPreviewIconPath(variantId, pen.id)
  if (iconPath) {
  ...
  // 未登记资源必须显式报错，不能清空图片后把缺失图元误报为加载成功。
  throw new Error(`光伏拓扑图片图元未登记：${variantId}/${pen.id}`)
}

/** 逐文件校验图元数、稳定编号与有限几何字段，避免损坏数据污染画布边界。 */
function validateSolarTopologyPens(
  pens: readonly Pen[],
  variantId: SolarTopologyVariantId,
  expectedPenCount: number,
): void {
  if (pens.length !== expectedPenCount) {
    throw new Error(`光伏拓扑 ${variantId} 图元数量异常：应为 ${expectedPenCount}，实际为 ${pens.length}。`)
  }
  const ids = new Set<string>()
  for (const pen of pens) {
    if (!pen.id) throw new Error(`光伏拓扑 ${variantId} 格式无效：存在无编号图元。`)
    if (ids.has(pen.id)) throw new Error(`光伏拓扑 ${variantId} 格式无效：图元编号重复 ${pen.id}。`)
    ids.add(pen.id)
    if (![pen.x, pen.y, pen.width, pen.height].every((value) => typeof value === 'number' && Number.isFinite(value))) {
      throw new Error(`光伏拓扑 ${variantId} 格式无效：图元 ${pen.id} 的坐标或尺寸不是有限数字。`)
    }
  }
}
 ...
 */
export async function loadSolarTopologyPreviewData(
  variantId: SolarTopologyVariantId = 'network-business-key-process',
  signal?: AbortSignal,
): Promise<Meta2dData> {
  const variant = SOLAR_TOPOLOGY_VARIANT_BY_ID.get(variantId)
  if (!variant) throw new Error(`光伏拓扑版本未登记：${variantId}`)

  let sourceData = sourceDataCache.get(variantId)
  if (!sourceData) {
    const response = await fetch(getSolarTopologyPreviewPublicAssetUrl(variant.topologyPath), {
      signal,
      cache: import.meta.env.DEV ? 'no-store' : 'force-cache',
    })
    if (!response.ok) throw new Error(`光伏拓扑 ${variantId} 加载失败：${response.status}`)
    const fetchedData = await response.json() as Partial<Meta2dData>
    if (!Array.isArray(fetchedData.pens)) throw new Error(`光伏拓扑 ${variantId} 格式无效：缺少图元数组。`)
    validateSolarTopologyPens(fetchedData.pens, variantId, variant.expectedPenCount)
  ...
  applySolarTopologySelectionPolicy(data.pens, variantId)
...
export function clearSolarTopologyPreviewDataCacheForTests(): void {
