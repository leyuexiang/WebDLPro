...
export const WIND_TOPOLOGY_SELECTABLE_LOCK = 1 as LockState

/** 对应二维组态引擎（Meta2D）的“完全禁用”：背景、分组框、文字和连线均跳过鼠标命中。 */
export const WIND_TOPOLOGY_BACKGROUND_LOCK = 10 as LockState

/**
 * 应用目标文件的显式选择清单。只有登记设备和工艺矩形可选择；热路径只做集合查询，
 * 不会把“带图片”直接等同于设备，也不会按标题、尺寸、位置或文件顺序重新分类。
 */
export function applyWindTopologySelectionPolicy(
  pens: Pen[],
  ...
  for (const pen of pens) {
    ...
    pen.locked = selectable ? WIND_TOPOLOGY_SELECTABLE_LOCK : WIND_TOPOLOGY_BACKGROUND_LOCK
