...
import {
  clearSwitchingStationTopologyPreviewDataCacheForTests,
  loadSwitchingStationTopologyPreviewData,
} from './switching-station-topology-preview-data'
import {
  SWITCHING_STATION_TOPOLOGY_BACKGROUND_LOCK,
...
function installTopologyFetch(): void {
  globalThis.fetch = vi.fn(async (input) => {
    const variant = SWITCHING_STATION_TOPOLOGY_VARIANTS.find((entry) => String(input).endsWith(entry.topologyPath))
...
describe('开关站拓扑独立数据与公共资源', () => {
  ...
  it('全部图片均映射到公共目录，设备可只读选择且背景与连线不可命中', async () => {
    ...
    for (const variant of SWITCHING_STATION_TOPOLOGY_VARIANTS) {
      const data = await loadSwitchingStationTopologyPreviewData(variant.id)
      const manifest = getSwitchingStationTopologyResourceManifest(variant.id)
      ...
      for (const [penId, path] of manifest.staticImagePathByPenId) {
        ...
        expect(data.pens.find((pen) => pen.id === penId)?.locked).toBe(SWITCHING_STATION_TOPOLOGY_SELECTABLE_LOCK)
      }
      for (const penId of manifest.processNodePenIds) {
        expect(data.pens.find((pen) => pen.id === penId)?.locked).toBe(SWITCHING_STATION_TOPOLOGY_SELECTABLE_LOCK)
      }
      expect(data.pens.filter((pen) => pen.name === 'line')
        .every((pen) => pen.locked === SWITCHING_STATION_TOPOLOGY_BACKGROUND_LOCK)).toBe(true)
      for (const penId of manifest.titleBackgroundPenIds) {
        expect(data.pens.find((pen) => pen.id === penId)?.locked).toBe(SWITCHING_STATION_TOPOLOGY_BACKGROUND_LOCK)
      }
    }
  })
  ...
  it('三层完整图移除顶层组合并保持所有子图元世界坐标', async () => {
    ...
    const variant = SWITCHING_STATION_TOPOLOGY_VARIANTS.find((entry) => entry.id === variantId)!
    const source = JSON.parse(readFileSync(resolve(topologyRoot, variant.topologyPath), 'utf8')) as Meta2dData
    const data = await loadSwitchingStationTopologyPreviewData(variantId)
  ...
  it('缓存返回深拷贝，重新切回不继承临时几何和图片修改', async () => {
    installTopologyFetch()
    const first = await loadSwitchingStationTopologyPreviewData('architecture')
    const sourceX = first.pens[0]!.x
    first.pens[0]!.x = 987654
    first.pens.find((pen) => pen.id === '1990437b')!.image = '污染地址'

    const second = await loadSwitchingStationTopologyPreviewData('architecture')
    expect(second.pens[0]!.x).toBe(sourceX)
    expect(second.pens.find((pen) => pen.id === '1990437b')?.image).not.toBe('污染地址')
