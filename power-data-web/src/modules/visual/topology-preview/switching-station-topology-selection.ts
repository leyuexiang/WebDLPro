...
export const SWITCHING_STATION_TOPOLOGY_SELECTABLE_LOCK = 1 as LockState
/** 背景、连线、分组和文字完全禁用命中，避免大面积背景截获设备点击。 */
export const SWITCHING_STATION_TOPOLOGY_BACKGROUND_LOCK = 10 as LockState

/**
 * 移除三层完整图中覆盖全图的顶层 combine（组合图元）。
 *
 * 该源文件唯独把 83 个主体图元放在一个覆盖全图的大组合内，Meta2D 点击任意子图元时会激活整组。
 * 加载时将所有直接子图元的比例坐标换算成世界坐标，然后删除该顶层组合。这个变换不改变
 * 图元顺序、视觉位置、连线端点和更深层组合；后续锁定策略仍会使背景、区域框和线段不可命中。
 */
export function removeSwitchingStationFullTopologyRootCombine(
  pens: Pen[],
  ...
  if (rootIndex < 0 || !root?.id) throw new Error('开关站三层完整图缺少顶层组合图元。')
  if (![root.x, root.y, root.width, root.height].every((value) => typeof value === 'number' && Number.isFinite(value))) {
    throw new Error(`开关站三层完整图顶层组合几何数据无效：${root.id}`)
  }
  ...
  for (const pen of pens) {
    ...
    if (![pen.x, pen.y, pen.width, pen.height].every((value) => typeof value === 'number' && Number.isFinite(value))) {
      throw new Error(`开关站三层完整图子图元几何数据无效：${pen.id ?? '未知图元'}`)
    }
 ...
 */
export function applySwitchingStationTopologySelectionPolicy(
  pens: Pen[],
  variantId: SwitchingStationTopologyVariantId,
): ReadonlySet<string> {
