...
import {
  ...
  type ConverterStationTopologyVariantId,
} from './converter-station-topology-variant-manifest'

/** 未交给二维组态引擎的安全源副本按版本缓存；再次切回不重复请求或解析。 */
const sourceDataCache = new Map<ConverterStationTopologyVariantId, Meta2dData>()

/** 生成换流站版本数据地址；图片始终通过公共资源工具生成，不访问源包站点路径。 */
export function getConverterStationTopologyPublicAssetUrl(
  relativePath: string,
  ...
  const assetRelativePath = `topology/converter-station-json-preview/${relativePath}`
 ...
 */
function localizePenImage(pen: Pen, variantId: ConverterStationTopologyVariantId): void {
  ...
  if (!staticPath) throw new Error(`换流站拓扑图片图元未登记：${variantId}/${pen.id}`)
  pen.image = getTopologySharedPublicAssetUrl(staticPath)
}

/** 图元数、编号和有限几何字段在进入画布前逐项校验，防止异常边界污染适应视图。 */
function validateConverterStationTopologyPens(
  pens: readonly Pen[],
  variantId: ConverterStationTopologyVariantId,
  expectedPenCount: number,
): void {
  if (pens.length !== expectedPenCount) {
    throw new Error(`换流站拓扑 ${variantId} 图元数量异常：应为 ${expectedPenCount}，实际为 ${pens.length}。`)
  }
  const ids = new Set<string>()
  for (const pen of pens) {
    if (!pen.id) throw new Error(`换流站拓扑 ${variantId} 格式无效：存在无编号图元。`)
    if (ids.has(pen.id)) throw new Error(`换流站拓扑 ${variantId} 格式无效：图元编号重复 ${pen.id}。`)
    ids.add(pen.id)
    if (![pen.x, pen.y, pen.width, pen.height].every((value) => typeof value === 'number' && Number.isFinite(value))) {
      throw new Error(`换流站拓扑 ${variantId} 格式无效：图元 ${pen.id} 的坐标或尺寸不是有限数字。`)
    }
  }
}
 ...
 */
export async function loadConverterStationTopologyPreviewData(
  variantId: ConverterStationTopologyVariantId = 'network-business-key-process',
  signal?: AbortSignal,
): Promise<Meta2dData> {
  const variant = CONVERTER_STATION_TOPOLOGY_VARIANT_BY_ID.get(variantId)
  if (!variant) throw new Error(`换流站拓扑版本未登记：${variantId}`)

  let sourceData = sourceDataCache.get(variantId)
  if (!sourceData) {
    const response = await fetch(getConverterStationTopologyPublicAssetUrl(variant.topologyPath), {
      signal,
      cache: import.meta.env.DEV ? 'no-store' : 'force-cache',
    })
    if (!response.ok) throw new Error(`换流站拓扑 ${variantId} 加载失败：${response.status}`)
    const fetchedData = await response.json() as Partial<Meta2dData>
    if (!Array.isArray(fetchedData.pens)) throw new Error(`换流站拓扑 ${variantId} 格式无效：缺少图元数组。`)
    validateConverterStationTopologyPens(fetchedData.pens, variantId, variant.expectedPenCount)
...
export function clearConverterStationTopologyPreviewDataCacheForTests(): void {
