...
- 总览按显式 `focusRegions` 框选已绑定三维模型的入口及其子节点，颜色按区域标识稳定分配且不显示区域名称；关键环节过滤视图不显示区域框。
- 节点直接使用正式拓扑图元，文字位于图元下方；未选中时不叠加常驻外框，选中时显示独立高对比反馈并增强关联路径；连线优先直连，冲突时最少绕行。
...
- [拓扑下钻关联覆盖层实施计划](../Docs/前端原子任务/拓扑图/拓扑下钻关联覆盖层实施计划.md)
