...
- `baselines/double-container-600x600.jpg`：最小测试画布，验证上下各半的双容器、拓扑控件与唯一画布仍可操作。
- `baselines/double-container-1280x720.jpg`：常规桌面视口，验证上方半区内三维 16:9 视口、下方等高拓扑区及无状态摘要画面。
...
- 更新基准前必须同时复核三维半区内的 16:9、上下容器等高、拓扑画布填满标题之外余高、唯一画布与浏览器控制台错误；正式内容只影响任务-053的业务验收，不改变本组件基准的结构职责。
